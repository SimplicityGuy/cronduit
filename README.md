...
| `:latest` | The most recent stable (non-rc) release -- currently `:1.2.0` | Only on stable release tags (`vX.Y.Z` with no `-rc.N` suffix) | Operators who always want the newest stable. Never bleeds rc or main-branch builds in |
| `:rc` | The most recent release candidate -- currently `:1.2.0-rc.4` (last rc before `v1.2.0` shipped; won't move again until the next milestone begins rc cycling) | Every rc push (`vX.Y.Z-rc.N`). Never moves on stable releases | Early adopters who want to exercise the next milestone before it ships |
...
flowchart LR
    ...
    PUSH_RC["push v1.2.0-rc.N tag"] --> RELEASE["release.yml"]
    RELEASE --> RC[":1.2.0-rc.N<br/>:rc"]

    PUSH_STABLE["push v1.2.0 tag"] --> RELEASE
    RELEASE --> STABLE[":1.2.0<br/>:1.2<br/>:1<br/>:latest"]
