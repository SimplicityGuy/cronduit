...
| Docker buildx layers (main-branch multi-arch push) | `docker/build-push-action@v6` + `type=gha` | `main-build.yml` | `main-build` | `scope=cronduit-main` | Dockerfile change, base image digest change, 7-day TTL |
| Docker buildx layers (compose-smoke standalone) | `docker/build-push-action@v6` + `type=gha` | `compose-smoke.yml` | `compose-smoke` | `scope=cronduit-compose-smoke` | Dockerfile change, base image digest change, 7-day TTL |
...
- The main-branch path (`main-build.yml`, which uses `docker/build-push-action@v6` with `scope=cronduit-main`) and the release path (`docker/build-push-action@v6` in `release.yml`) are the throughput-critical cache consumers, and both are explicitly cached (main via `scope=cronduit-main`, release via `scope=cronduit-release`).
...
2. **Check the scope string.** For Docker caches, `grep -rn 'scope=' .github/workflows/` should enumerate every scope across all workflow files (ci.yml, release.yml, main-build.yml, compose-smoke.yml). Two jobs sharing a scope (except the single-step multi-platform release case documented above) will corrupt each other's layer cache.
...
5. **Scope strings match doc.** `grep -rn 'scope=' .github/workflows/` matches the "Cache inventory" table in this file. If the two drift, this doc is authoritative and the workflow files must be updated to match — not the other way around.
