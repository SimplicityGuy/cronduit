...
|------|-------|------|--------|-------------|-------------------|
| Body | `--cd-text-base` | 0.9rem (~14.4px) | 400 | 1.6 | Panel row values, bucket count text, "last seen" timestamps |
| Label | `--cd-text-xs` | 0.65rem (~10.4px) | 700 | 1.5 | Panel row labels (uppercase, `letter-spacing: 0.1em`); histogram bucket labels under each bar |
| Subheading | `--cd-text-sm` | 0.8rem (~12.8px) | 400 | 1.5 | Panel row helper text ("(suppressed below 5 samples)"); histogram x-axis caption; "Recent codes" sub-table cells |
| Heading | `--cd-text-lg` | 1.25rem (~20px) | 700 | 1.4 | Card heading ("Exit Code Distribution"; "Failure Context") — matches existing job-detail "Configuration" + "Duration" siblings |

Letter spacing: labels = `0.1em` (uppercase), heading = `-0.02em` (matches existing card titles), all body = default (0).

> **Critical reuse:** the existing job-detail Duration card uses `font-size: var(--cd-text-xl); font-weight: 700; letter-spacing: -0.02em` on its title BUT note that in `job_detail.html` line 72 the Duration card uses `--cd-text-xl`, while `Run History` on line 98 uses `--cd-text-lg`. The exit-code card heading mirrors **the Duration sibling** (`--cd-text-xl; font-weight: 700; letter-spacing: -0.02em`) so the two stats cards visually balance. The failure-context panel heading uses `--cd-text-lg; font-weight: 700` to match the secondary "Run History" / "Log Output" tier (it's not a primary stat — it's an inline diagnostic).
...
| Accent (10%) — RESERVED FOR | `--cd-text-accent` (= `--cd-green`) | `#34d399` / `#059669` | (1) "view last successful run" hyperlink in panel; (2) success-rate stat-badge label "✓ N% success" on the exit-code card; (3) focus rings (already on `--cd-border-focus`); (4) `<summary>` caret hover/focus |
...
1. `<a>` link to last successful run inside the failure-context panel (text + hover underline)
2. Success-rate stat badge label ("✓ 87% success") on the exit-code card
3. Focus rings on the `<details>` summary, on histogram bars (when keyboard-navigable), and on the link
4. `<summary>` caret + label color when the panel is in `[open]` state
...
| `.cd-fctx-summary` | `display: flex`, `align-items: center`, `gap: var(--cd-space-3)`, `padding: var(--cd-space-4) var(--cd-space-6)`, `background: var(--cd-bg-surface-raised)`, `cursor: pointer`, `font-size: var(--cd-text-lg)`, `font-weight: 700`, `letter-spacing: -0.02em`, `list-style: none`, `user-select: none` |
...
| `.cd-exit-stat` | `display: flex`, `flex-direction: column`, `gap: 2px` |
...
--cd-text-xs, --cd-text-sm, --cd-text-base, --cd-text-lg, --cd-text-xl
...
- [ ] Dimension 3 Color: PASS (60/30/10 split declared with explicit accent reserved-for list; 10-bucket → token mapping locked; zero new tokens)
- [ ] Dimension 4 Typography: PASS (4 size roles, 2 weights only, JetBrains Mono everywhere, line-heights from existing scale, no new sizes)
- [ ] Dimension 5 Spacing: PASS (4px-base scale reused verbatim; no exceptions; touch target ≥44px on `<summary>`)
