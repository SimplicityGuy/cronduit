...
---
*Last updated: 2026-04-26 — Phase 15 (Foundation Preamble) complete: workspace bumped to `1.2.0`, `cargo-deny` CI preamble landed warn-only (deny.toml + just deny + lint-job step with `continue-on-error: true`), webhook delivery worker scaffolding wired into `SchedulerLoop` with bounded `mpsc(1024)` + `try_send` non-blocking emit at `finalize_run` step 7d + `cronduit_webhook_delivery_dropped_total` counter. WH-02 scheduler-survival contract locked at the test boundary (`tests/v12_webhook_queue_drop.rs`, `tests/v12_webhook_scheduler_unblocked.rs`). FOUND-15, FOUND-16, WH-02 validated. Next: Phase 16 (Failure-Context Schema + run.rs:277 bug fix). Previous: 2026-04-25 — v1.2 milestone "Operator Integration & Insight" kicked off via `/gsd-new-milestone`. Five features: webhooks (override pattern), failure context on run detail (time + image-digest + config-hash deltas), per-job exit-code histogram, job tagging (UI-only), custom Docker labels (SEED-001). Cross-run log search and job concurrency/queuing punted to v1.3. Phase numbering continues from v1.1's last (Phase 14) → v1.2 starts at Phase 15. 2026-04-24 — v1.1 milestone closed.*
