...
---
*Last updated: 2026-04-13 — Phase 6 complete (7/7 plans): Live events, metrics, retention, release engineering. SSE log streaming on `/events/runs/:id/logs`, Prometheus `/metrics` with five cronduit families eagerly described at boot (GAP-1 closed), daily retention pruner with startup tracing (GAP-2 closed), README + THREAT_MODEL + docker-compose quickstart, multi-arch release workflow (amd64+arm64 via cargo-zigbuild), `.gitignore` hygiene for UAT scratch configs, and a new `compose-smoke` CI job that builds the PR's image locally, rewrites compose in the runner workspace, and asserts `/health` + job load. OPS-04 (ports: vs expose:) deferred to Phase 7; OPS-05 human UAT deferred to Phase 8.*
