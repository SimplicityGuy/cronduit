...
| CONF-04 | 01-02 + quick/260414-gbf | per-job use_defaults = false | ✓ SATISFIED (retroactive — see issue #20) | `JobConfig.use_defaults: Option<bool>` field present from Phase 1; actual enforcement via `src/config/defaults.rs::apply_defaults` early return lands in the issue #20 fix. Coverage: `apply_defaults_use_defaults_false_disables_merge` unit test + `use_defaults_false_disables_merge` integration test in `tests/defaults_merge.rs` |
...
| CONF-06 | 01-02 + quick/260414-gbf | Job-level field overrides defaults | ✓ SATISFIED (retroactive — see issue #20) | Config struct hierarchy from Phase 1; actual override precedence enforced by `src/config/defaults.rs::apply_defaults` in the issue #20 fix. Coverage: `apply_defaults_job_override_wins_*` unit tests + `job_override_wins_image`/`job_override_wins_network` integration tests in `tests/defaults_merge.rs` |
